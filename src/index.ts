...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    const shellSignal: { connect: (fn: (s: unknown, a: any) => void) => void } | null =
      typeof labShell.activeChanged?.connect === 'function' ? labShell.activeChanged :
      typeof labShell.currentChanged?.connect === 'function' ? labShell.currentChanged :
      null;
    if (shellSignal) {
      shellSignal.connect((_: unknown, args: any) => {
        ...
        if (notebookTracker.has(newWidget as any)) {
          const path: string = (newWidget as any).context?.path ?? '';
          if (path) dispatchNotebookActivated(path);
          return;
        }

        const filePath: string = (newWidget as any)?.context?.path ?? '';
        if (filePath) {
    ...
    notebookTracker.currentChanged.connect(async (_, widget) => {
      if (!widget) return;
      const notebookPath = widget.context?.localPath ?? '';
      if (!notebookPath) return;
      try {
