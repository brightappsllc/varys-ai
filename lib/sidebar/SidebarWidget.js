...
const DSAssistantChat = ({ apiClient, notebookReader, cellEditor, notebookTracker, openFile, reloadFile, }) => {
     ...
     */
    const diffStoreRef = useRef(new Map());
    ...
    const handleAccept = (operationId) => {
        ...
        if (_ae) {
            diffStoreRef.current.set(operationId, Object.assign(Object.assign({}, _ae), { resolved: 'accepted' }));
            bumpDiffStore();
        }
    };
    const handleUndo = (operationId) => {
        ...
        if (_ue) {
            diffStoreRef.current.set(operationId, Object.assign(Object.assign({}, _ue), { resolved: 'undone' }));
            bumpDiffStore();
        }
    };
    // -------------------------------------------------------------------------
