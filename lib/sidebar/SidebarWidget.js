    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                label: 'Indexing & Docs',
                icon: (React.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "none", stroke: "currentColor", strokeWidth: "1.3" },
                    React.createElement("path", { d: "M3 3h9M3 7h9M3 11h5" }))),
            },
            {
                id: 'tags',
                label: 'Tags',
...
const FileAgentConfigPanel = ({ notebookPath, apiClient, onClose }) => {
    var _a, _b, _c;
    const [values, setValues] = useState({});
...
// Chat component
// ---------------------------------------------------------------------------
const DSAssistantChat = ({ apiClient, notebookReader, cellEditor, notebookTracker, openFile, reloadFile, }) => {
    var _a, _b, _c, _d;
     ...
     */
    const [diffStore, setDiffStore] = useState(new Map());
    ...
    explicitPath) => {
        ...
        const saved = msgs
            ...
            .map(m => (Object.assign({ id: m.id, role: m.role, content: m.content, timestamp: m.timestamp.toISOString() }, (m.thoughts ? { thoughts: m.thoughts } : {}))));
        const now = new Date().toISOString();
    ...
    const _restoreFromCache = (path) => {
        ...
        setMessages(lastThread && lastThread.messages.length > 0
            ? lastThread.messages.map(m => (Object.assign({ id: m.id, role: m.role, content: m.content, timestamp: new Date(m.timestamp), fromHistory: true }, (m.thoughts ? { thoughts: m.thoughts } : {}))))
            : []);
            ...
            if (isAgentCommand) {
                ...
                if (knownBuiltin) {
                    ...
                    if (parsed.command === '/index') {
                        await handleIndexCommand((_d = (_c = parsed.rest) === null || _c === void 0 ? void 0 : _c.trim()) !== null && _d !== void 0 ? _d : '');
                        return;
                    }
                    ...
                    if (parsed.command === '/ask' && parsed.rest) {
                        ...
                        slashCommand = parsed.command;
                        message = (_f = (_e = parsed.rest) === null || _e === void 0 ? void 0 : _e.trim()) !== null && _f !== void 0 ? _f : '';
                        // Don't return early — fall through to the main task flow below.
                    }
        ...
        try {
            ...
            if (slashCommand === '/report') {
                ...
                try {
                    ...
                    addMessage('system', `Report generation failed: ${(_g = err === null || err === void 0 ? void 0 : err.message) !== null && _g !== void 0 ? _g : err}`);
                }
                return;
            }
            // ────────────────────────────────────────────────────────────────
            ...
            let thoughtsAccum = '';
            const response = await apiClient.executeTaskStreaming(Object.assign(Object.assign(Object.assign(Object.assign({ message, notebookContext: context, chatHistory, variables: resolvedVariables }, (slashCommand ? { command: slashCommand } : {})), { cellMode: (notebookAware || !!currentFilePathRef.current) ? cellMode : 'chat' }), (reasoningModeRef.current !== 'off' ? { reasoningMode: reasoningModeRef.current } : {})), (imageModeRef.current ? { imageMode: imageModeRef.current } : {})), 
            ...
            if (response.cellInsertionMode === 'composite' && response.compositePlan) {
                ...
                await runCompositePipeline((_h = response.compositeName) !== null && _h !== void 0 ? _h : 'pipeline', response.compositePlan);
                return;
            }
            ...
            if (!response.requiresApproval) {
                for (let i = 0; i < response.steps.length; i++) {
                    ...
                    if (shouldRun) {
                        const notebookIndex = (_j = stepIndexMap.get(i)) !== null && _j !== void 0 ? _j : step.cellIndex;
            ...
            const op = {
                ...
                description: (_k = response.summary) !== null && _k !== void 0 ? _k : `Created/modified ${response.steps.length} cell(s)`,
                diffs
            };
    ...
    const handleAccept = (operationId) => {
        ...
        setDiffStore(prev => {
            ...
            if (!entry)
                return prev;
            return new Map(prev).set(operationId, Object.assign(Object.assign({}, entry), { resolved: 'accepted' }));
        });
    };
    const handleUndo = (operationId) => {
        ...
        setDiffStore(prev => {
            ...
            if (!entry)
                return prev;
            return new Map(prev).set(operationId, Object.assign(Object.assign({}, entry), { resolved: 'undone' }));
        });
    };
    // -------------------------------------------------------------------------
    ...
    const handleSwitchThread = (threadId) => {
        ...
        const restored = thread.messages.length > 0
            ? thread.messages.map(m => (Object.assign({ id: m.id, role: m.role, content: m.content, timestamp: new Date(m.timestamp), fromHistory: true }, (m.thoughts ? { thoughts: m.thoughts } : {}))))
            : [{
                    id: `welcome-${threadId}`,
                    role: 'system',
    ...
    const handleDuplicateThread = (threadId) => {
        ...
        const restored = copy.messages.length > 0
            ? copy.messages.map(m => (Object.assign({ id: m.id, role: m.role, content: m.content, timestamp: new Date(m.timestamp), fromHistory: true }, (m.thoughts ? { thoughts: m.thoughts } : {}))))
            : [{
                    id: `welcome-${copy.id}`,
                    role: 'system',
    ...
    return (React.createElement("div", { className: `ds-assistant-sidebar ds-chat-${chatTheme}` },
        React.createElement("div", { className: "ds-assistant-header" },
            React.createElement("span", { className: "ds-assistant-title" },
                React.createElement("span", { className: "ds-varys-spider" }, "\uD83D\uDD77\uFE0F"),
                " Varys ",
                React.createElement("span", { className: "ds-varys-version" }, "v0.6.0")),
            ...
            messages.map(msg => {
                ...
                return (React.createElement(React.Fragment, { key: msg.id },
                    ...
                    msg.operationId && diffStore.has(msg.operationId) && (() => {
                        ...
                        return (React.createElement(DiffView, { operationId: msg.operationId, diffs: entry.diffs, onAccept: handleAccept, onUndo: handleUndo, resolved: entry.resolved }));
                    })()));
            }),
        ...
        pendingOps.some(op => !op.resolved) && (React.createElement("div", { className: "ds-assistant-pending-ops" }, pendingOps.filter(op => !op.resolved).map(op => (React.createElement(DiffView, { key: op.operationId, operationId: op.operationId, description: op.description, diffs: op.diffs, onAccept: handleAccept, onUndo: handleUndo }))))),
        agentResultsReady && (React.createElement("div", { className: "ds-agent-results" },
            React.createElement("div", { className: "ds-agent-results-header" },
                React.createElement("span", { className: "ds-agent-results-title" }, "Varys File Agent"),
                React.createElement("button", { className: `ds-agent-config-btn${agentConfigOpen ? ' ds-agent-config-btn--active' : ''}`, onClick: () => setAgentConfigOpen(o => !o), title: "Project settings (.jupyter-assistant/local_varys.env)" }, "\u2699")),
            agentConfigOpen && (React.createElement(FileAgentConfigPanel, { notebookPath: currentNotebookPath || currentFilePath, apiClient: apiClient, onClose: () => setAgentConfigOpen(false) })),
            agentToolError && (React.createElement(AgentToolErrorBanner, { error: agentToolError, onOpenAgentSettings: () => {
                    setSettingsOpenToAgent(true);
                } })),
            agentFilesRead.length > 0 && (React.createElement("div", { className: "ds-agent-context-chips" },
                React.createElement("span", { className: "ds-agent-context-label" }, "Context used:"),
                agentFilesRead.map((f, i) => (React.createElement("span", { key: i, className: "ds-agent-context-chip", title: f }, f))))),
            agentFileChanges.map(fc => (React.createElement(FileChangeCard, { key: fc.change_id, event: fc, operationId: agentOperationId, apiBaseUrl: "", xsrfToken: getXsrfToken(), onResolved: (changeId, accepted) => {
                    ...
                    const allDone = agentFileChanges.every(fc => newResolved[fc.change_id] !== undefined);
                    if (allDone) {
                        // Build a code-block summary for each accepted change.
                        const acceptedChanges = agentFileChanges.filter(fc => newResolved[fc.change_id] === true && fc.new_content);
                        if (acceptedChanges.length > 0) {
                            const summaryParts = acceptedChanges.map(fc => {
                                var _a, _b, _c, _d, _e;
                                const fname = (_a = fc.file_path.split('/').pop()) !== null && _a !== void 0 ? _a : fc.file_path;
                                const ext = fname.includes('.') ? fname.split('.').pop().toLowerCase() : '';
                                const lang = (_b = { py: 'python', js: 'javascript', ts: 'typescript',
                                    tsx: 'tsx', jsx: 'jsx', md: 'markdown', sh: 'bash',
                                    yml: 'yaml', yaml: 'yaml', json: 'json', rs: 'rust',
                                    go: 'go', rb: 'ruby', r: 'r', sql: 'sql' }[ext]) !== null && _b !== void 0 ? _b : ext;
                                // Show only lines that were added (simple line-diff).
                                const origLines = ((_c = fc.original_content) !== null && _c !== void 0 ? _c : '').split('\n');
                                const newLines = ((_d = fc.new_content) !== null && _d !== void 0 ? _d : '').split('\n');
                                const addedLines = newLines.filter(l => !origLines.includes(l) && l.trim() !== '');
                                const codeBlock = addedLines.length > 0 && addedLines.length <= 60
                                    ? addedLines.join('\n')
                                    : ((_e = fc.new_content) !== null && _e !== void 0 ? _e : '');
                        ...
                        if (activePath) {
                            ...
                            if (entry)
                                historyCacheRef.current.set(activePath, Object.assign(Object.assign({}, entry), { agentPanel: undefined }));
                        }
                    }
                } }))),
                ...
                React.createElement("button", { className: "ds-assistant-btn ds-assistant-btn-accept", onClick: async () => {
                        ...
                        for (const fc of agentFileChanges) {
                            if (agentResolved[fc.change_id] === undefined) {
                                ...
                                catch ( /* ignore per-item errors in bulk accept */_a) { /* ignore per-item errors in bulk accept */ }
                            }
                        }
                    } }, "\u2713 Accept All"),
                React.createElement("button", { className: "ds-assistant-btn ds-assistant-btn-undo", onClick: async () => {
                        ...
                        for (const fc of agentFileChanges) {
                            if (agentResolved[fc.change_id] === undefined) {
                                ...
                                catch ( /* ignore per-item errors in bulk reject */_a) { /* ignore per-item errors in bulk reject */ }
                            }
                        }
                    } }, "\u2715 Reject All"))),
            agentIncomplete && (React.createElement("div", { className: "ds-agent-incomplete-banner" }, "\u26A0 Task reached the turn limit \u2014 results may be incomplete.")),
        ...
        React.createElement("div", { className: "ds-assistant-input-area" },
            ...
            React.createElement("div", { className: "ds-input-body" },
                React.createElement("div", { className: "ds-nb-ctx-row" },
                    ...
                    extractCellRefs(input).map(ref => (React.createElement("span", { key: ref, className: "ds-nb-ctx-chip ds-nb-ctx-chip--on ds-cell-ref-ctx-chip", title: `"${ref}" referenced in your query`, "aria-label": ref }, ref))),
                ...
                React.createElement("div", { ref: textareaRef, role: "textbox", "aria-multiline": "true", contentEditable: isLoading ? 'false' : 'true', className: "ds-assistant-input ds-assistant-ce", style: { minHeight: MIN_INPUT_HEIGHT, maxHeight: inputHeight }, "data-placeholder": contextChip ? `Describe your edit for ${contextChip.label}…` : "Ask Varys… (use @varName · /command · Enter to send)", suppressContentEditableWarning: true, onInput: () => {
            ...
            React.createElement("div", { className: "ds-assistant-input-bottom" },
                ...
                React.createElement("div", { className: "ds-input-controls" }, (notebookAware || !!currentFilePath) && (React.createElement("select", { className: "ds-cell-mode-select", value: cellMode, title: CELL_MODE_TITLE[cellMode], onChange: e => {
