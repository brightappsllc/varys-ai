...
function nodeBackground(anomalies, selected, upstream, downstream) {
    if (selected)
...
export const GraphNode = ({ node, layout, selected, upstream, downstream, dimmed, onClick, }) => {
    ...
    return (React.createElement("g", { className: "ds-graph-node", transform: `translate(${x},${y})`, style: { cursor: 'pointer', opacity, transition: 'opacity 0.15s' }, onClick: () => onClick(node.cellUuid) },
        ...
        React.createElement("rect", { x: 0, y: 0, width: NODE_WIDTH, height: NODE_HEIGHT, rx: 8, fill: bg, stroke: selected ? ringColor : hasUnexecutedBorder ? '#E8891A' : 'var(--jp-border-color1)', strokeWidth: selected ? 2 : hasUnexecutedBorder ? 2 : 1, strokeDasharray: hasUnexecutedBorder && !selected ? '5,3' : undefined }),
        React.createElement("foreignObject", { x: 0, y: 0, width: NODE_WIDTH, height: NODE_HEIGHT },
            React.createElement("div", { className: "ds-graph-node-inner", style: {
                ...
                React.createElement("span", { className: "ds-graph-cell-badge", style: {
                        position: 'absolute',
                        top: 4,
                        left: 6,
                        fontSize: 9,
