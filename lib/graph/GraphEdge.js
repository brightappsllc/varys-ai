...
function getEdgeColor(edge) {
    var _a, _b, _c;
    if (edge.anomaly)
        return (_a = ANOMALY_COLORS[edge.anomaly]) !== null && _a !== void 0 ? _a : 'var(--jp-border-color2)';
    return (_c = EDGE_TYPE_COLORS[(_b = edge.edgeType) !== null && _b !== void 0 ? _b : 'dependency']) !== null && _c !== void 0 ? _c : 'var(--jp-border-color2)';
}
function getArrowId(edge) {
    if (edge.anomaly)
        return `${ARROW_ID_PREFIX}${edge.anomaly}`;
    if (edge.edgeType === 'redefines')
        return `${ARROW_ID_PREFIX}redefines`;
    if (edge.edgeType === 'reimport')
        return `${ARROW_ID_PREFIX}reimport`;
...
export const GraphEdge = ({ edge, layout, nodeLayouts, zoom, dimmed }) => {
    ...
    return (React.createElement("g", { className: "ds-graph-edge", style: { opacity: dimmed ? 0.3 : 1, transition: 'opacity 0.15s' } },
        React.createElement("path", { d: pathData, fill: "none", stroke: color, strokeWidth: isThick ? 2 : 1.5, markerEnd: `url(#${arrowId})`, strokeDasharray: isDashed ? '6,4' : undefined }),
