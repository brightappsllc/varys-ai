...
export class APIClient {
    ...
    async saveChatThread(notebookPath, thread) {
        const response = await fetch(`${this.baseUrl}/chat-history`, {
            ...
            body: JSON.stringify({
                notebookPath,
                thread: Object.assign(Object.assign(Object.assign({ id: thread.id, name: thread.name, created_at: thread.createdAt, messages: thread.messages }, (thread.tokenUsage ? { token_usage: thread.tokenUsage } : {})), (thread.notebookAware !== undefined ? { notebook_aware: thread.notebookAware } : {})), (thread.cellMode !== undefined ? { cell_mode: thread.cellMode } : {})),
            }),
        });
        if (!response.ok)
