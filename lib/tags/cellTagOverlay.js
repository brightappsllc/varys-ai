...
function _buildPresetsFromLibrary(library) {
    ...
    for (const tag of library.tags) {
        if (!byTopic[tag.topic])
            byTopic[tag.topic] = [];
        byTopic[tag.topic].push(tag.value);
    }
    return Object.entries(byTopic).map(([category, tags]) => ({ category, tags }));
}
async function _fetchPresets(baseUrl) {
    var _a, _b;
    if (_cachedPresets)
        return _cachedPresets;
    try {
        const xsrf = (_b = ((_a = document.cookie.match('(?:^|;)\\s*_xsrf=([^;]*)')) !== null && _a !== void 0 ? _a : [])[1]) !== null && _b !== void 0 ? _b : '';
...
// ── [+] dropdown ───────────────────────────────────────────────────────────
function showAddTagDropdown(anchor, tracker, cellIndex, currentTags, refresh, onTagsChanged, presets = _FALLBACK_PRESETS) {
    closeDropdown();
    const applyTag = (tag) => {
        var _a;
        const nb = (_a = tracker.currentWidget) === null || _a === void 0 ? void 0 : _a.content;
...
// ── Main render ────────────────────────────────────────────────────────────
function renderOverlays(tracker, refresh, onAutoTag, onTagsChanged, baseUrl = '/varys') {
    var _a, _b;
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (onAutoTag) {
            ...
            if (isPending) {
                ...
                autoBtn.innerHTML = `<svg width="9" height="9" viewBox="0 0 24 24" fill="none"
 ...
 */
export function initCellTagOverlay(tracker, onAutoTag, onTagsChanged, baseUrl = '/varys') {
    // Pre-fetch the tag library so the dropdown is ready on first open.
    _fetchPresets(baseUrl).catch(() => { });
