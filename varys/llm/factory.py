...
def create_bg_task_provider(settings: Dict[str, Any]) -> "BaseLLMProvider | None":
    """Return a provider configured for Background Tasks (DS_BG_TASK_PROVIDER +
    {PROVIDER}_BG_TASK_MODEL).
    ...
    provider_name = (
        settings.get("ds_assistant_bg_task_provider", "")
        or os.environ.get("DS_BG_TASK_PROVIDER", "")
        # backward-compat: honour old env var names
    ...
    bg_model = (
        settings.get(f"ds_assistant_{provider_name}_bg_task_model", "")
        or os.environ.get(f"{provider_name.upper()}_BG_TASK_MODEL", "")
        # backward-compat
    ...
    try:
        _sync_credentials(settings)
        _check_api_key(provider_name, settings)
        # Cache under "bg_task" to avoid colliding with chat/completion cache entries
        key = _cache_key(provider_name, "bg_task", bg_model, settings)
        if key in _provider_cache:
            return _provider_cache[key]
        provider = _build_provider(provider_name, "chat", bg_model, settings)
        _provider_cache[key] = provider
        return provider
    except Exception as exc:
        log.debug("create_bg_task_provider: %s", exc)
        return None
